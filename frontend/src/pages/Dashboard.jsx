...
import { 
    Home, PlusCircle, Users, User, LogOut, 
    Download, Activity, Edit2, Trophy, Trash2, Zap, Target 
} from 'lucide-react';
...
const Dashboard = () => {
    ...
    const [userData, setUserData] = useState({
        username: 'User', weight: 0, height: 0, age: 0, goal: 'Fitness', calories: 0, macros: []
    });
    ...
    const fetchData = async () => {
        ...
        try {
            const [profileRes, prsRes] = await Promise.all([
                ...
                axios.get(`${API_BASE_URL}/api/get-prs/`, {
                    headers: { 'Authorization': `Token ${token}` }
                })
            ]);

            setUserData(profileRes.data);
            setUserPrs(prsRes.data);
            setLoading(false);
        } catch (error) {
    ...
    const handleGenerateDiet = async () => {
        ...
        try {
            ...
            link.href = url;
            link.setAttribute('download', `${userData.username}_FitLift_Plan.pdf`);
        ...
        <div className="flex min-h-screen bg-black text-white font-sans">
            ...
            <main className="flex-1 ml-64 p-10">
                <header className="flex justify-between items-center mb-12">
                    ...
                    <button onClick={() => navigate('/profile')} className="flex items-center gap-3 bg-orange-600 px-6 py-3 rounded-2xl font-black uppercase text-[10px] tracking-widest hover:bg-orange-700 transition-all shadow-lg shadow-orange-600/20">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-10">
                    <div className="bg-zinc-900/40 backdrop-blur-sm border border-white/5 p-8 rounded-[2.5rem] lg:col-span-1">
                        <p className="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-500 mb-6 flex items-center gap-2">
                           <Target size={12} className="text-orange-600" /> Health Category
                        </p>
                        ...
                        <p className="text-[9px] font-bold text-zinc-500 uppercase tracking-widest">
                            Ideal Weight: {Math.round(18.5 * (userData.height/100)**2)} - {Math.round(24.9 * (userData.height/100)**2)} KG
                        </p>
                    </div>
                    <StatCard title="Daily Calories" value={userData.calories} desc="Est. TDEE" color="text-orange-600" />
                    <StatCard title="Body Weight" value={`${userData.weight}kg`} desc={userData.goal} color="text-blue-500" />
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-10">
                    <div className="bg-zinc-900/40 backdrop-blur-md border border-white/5 p-8 rounded-[2.5rem] relative min-h-[450px]">
                        <h3 className="text-xs font-black uppercase tracking-[0.2em] text-zinc-500 mb-4 flex items-center gap-2">
                            <Activity size={14} className="text-orange-600" /> Macro Breakdown
                        </h3>
                        ...
                        {userData.macros && userData.macros.length > 0 ? (
                            <div className="flex flex-col items-center">
                                <div className="h-[250px] w-full relative">
                                    <ResponsiveContainer width="100%" height="100%">
                                        <PieChart>
                                            <Pie data={userData.macros} cx="50%" cy="50%" innerRadius={70} outerRadius={90} paddingAngle={8} dataKey="value" stroke="none">
                                                {userData.macros.map((entry, index) => (
                                                    <Cell key={`cell-${index}`} fill={entry.fill} />
                                                ))}
                                            </Pie>
                                            <Tooltip contentStyle={{ backgroundColor: '#18181b', border: 'none', borderRadius: '10px', fontSize: '10px', fontWeight: 'bold' }} />
                                        </PieChart>
                                    </ResponsiveContainer>
                                ...
                                <div className="grid grid-cols-3 gap-4 w-full mt-6 pt-6 border-t border-white/5">
                                    {userData.macros.map((macro, index) => (
                                        <div key={index} className="text-center">
                                            <p className={`text-[10px] font-black uppercase tracking-widest ${macro.name === 'Protein' ? 'text-orange-600' : 'text-zinc-500'}`}>{macro.name}</p>
                            ...
                            <p className="text-center py-20 text-zinc-600 italic uppercase text-[10px] font-bold tracking-widest">Loading Macros...</p>
                        )}
                    </div>
                            ...
                            {userPrs.length > 0 ? userPrs.map((pr, index) => (
                                <div key={index} className="group flex items-center justify-between bg-black/40 p-5 rounded-2xl border border-white/5 hover:border-orange-600/30 transition-all">
                                    ...
                                    <div className="flex items-center gap-4">
                                        ...
                                        <button onClick={() => handleDeletePR(pr.id)} className="text-zinc-700 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-all p-2">
                                            <Trash2 size={16} />
                                        </button>
                                    </div>
                                </div>
                            )) : (
                                <p className="text-zinc-600 italic font-bold uppercase text-[10px] text-center py-20">No records found. Push harder!</p>
                            )}
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-gradient-to-r from-zinc-900 to-orange-900/20 border border-orange-600/20 p-10 rounded-[2.5rem] flex items-center justify-between relative overflow-hidden">
                    <div className="absolute top-0 right-0 p-4 opacity-5"><Download size={120} /></div>
                    <div className="flex items-center gap-8">
                        ...
                        <div>
                            <h2 className="text-3xl font-black uppercase italic tracking-tighter text-white leading-tight">AI Personal Diet Plan</h2>
                            <p className="text-zinc-400 text-[10px] font-bold uppercase tracking-widest mt-1">Powered by Gemini 1.5 Flash • Customized for {userData.username}</p>
                        </div>
                    </div>
...
const StatCard = ({ title, value, desc, color }) => (
    <div className="bg-zinc-900/40 backdrop-blur-sm border border-white/5 p-8 rounded-[2rem] transition-all hover:border-white/10">
        ...
        <h4 className="text-5xl font-black italic tracking-tighter mb-2 leading-none">{value}</h4>
        <p className={`text-[10px] font-bold uppercase ${color} tracking-widest mt-2`}>{desc}</p>
    </div>
);
